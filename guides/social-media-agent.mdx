...
pip install "agno[infra,tools]" openai exa_py python-dotenv
...
### 2c. Combine Your Tools

```python
# Create your complete tools list
tools = [x_tools, exa_tools]
print(f"Total tools configured: {len(tools)}")
```

**Test your tools (optional):**

```python
# Test X tool (uncomment to test)
# x_result = x_tools.search_tweets("AI sentiment", max_results=5)
# print(f"X tool test: Found {len(x_result)} tweets")

# Test Exa tool (uncomment to test) 
# exa_result = exa_tools.search("AI market analysis")
# print(f"Exa tool test: Found {len(exa_result)} web results")
```
...
- `name`: Identifies your agent in logs and API calls
- `model`: The AI that powers your agent's reasoning
- `tools`: The capabilities your agent can use to gather data
- `instructions`: The methodology and strategy for analysis
- `markdown=True`: Enables rich formatting in reports
- `show_tool_calls=True`: Shows which tools are being used for transparency

### 4c. Create the Analysis Function
