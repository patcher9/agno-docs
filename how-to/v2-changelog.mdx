...
<Accordion title="Updates to Agent Class">
  ...
  - `send_media_to_model` -> `True` by default. Set to False if you don't want to send the media to the model.
  - `store_media` -> `True` by default. Set to False if you don't want to store the media in the `RunOutput`.
