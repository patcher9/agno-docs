...
Building context has become about answering this question: "What set of information is most likely to achieve the desired outcome?"
...
Effective context engineering involves iteratively refining the system message, experimenting with different descriptions and instructions, and leveraging features like input/output schemas, member delegation, or tool integrations. 
...
The system message is created using `description`, `instructions`, member information, and a number of other settings. The team leader system message includes member information, delegation instructions, and coordination guidelines. For example:
...
| `enable_agentic_knowledge_filters` | `bool`      | `False` | If True, let the team choose the knowledge filters. See [knowledge](/concepts/knowledge/filters/introduction) for more information.                                       |
...
<Tip>
This example is exhaustive to illustrate what is possible with the system message, however in practice you would only use some of these settings.
</Tip>
...
In this example, we add additional context to the system message that the team has access to specific tables in a database.
...
You can add a list of member interactions to the request that goes to a team member with `share_member_interactions`.
...
This adds information like the following excerpt to the task that goes to a team member:
```
<member interactions>
...
If you have `enable_agentic_memory` enabled on your team, the team gets the ability to create/update user memories using tools.
...
Learn about agentic knowledge filters in more detail in the [knowledge filters](/concepts/knowledge/filters/introduction) section.
...
By default, the user message is built using the `input` sent to the `Team.run()` or `Team.print_response()`.
...
For teams, the member information, delegation instructions, and coordination guidelines are typically static and good candidates for caching.
