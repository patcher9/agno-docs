...
In [**Coordinate Mode**](/concepts/teams/modes/coordinate), the team leader delegates tasks to team members and synthesizes their outputs into a single cohesive response.
...
<Note>
  In `async` execution, if more than one member is delegated to at once by the
  team leader, these members are run concurrently.
</Note>

### Route Mode

In [**Route Mode**](/concepts/teams/modes/route), the team leader forwards your run input to the most appropriate team member based on the content of the input. The member's response is returned directly to the user and the team leader doesn't interpret/transform the response.

<Note>
  In `async` execution, if more than one member is forwarded to at once by the
  team leader, these members are run concurrently.
</Note>

### Collaborate Mode

In [**Collaborate Mode**](/concepts/teams/modes/collaborate), all team members are given the same task and the team leader synthesizes their outputs into a cohesive response.

<Note>In `async` execution, all the members are run concurrently.</Note>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="fire"
    iconType="duotone"
    href="/concepts/teams/structured-input-output"
  >
