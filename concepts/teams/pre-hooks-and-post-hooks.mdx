...
- `run_input`: The input provided to the Team when invoking the run.
- `session`: The `TeamSession` object representing the contextual session.
...
You can see some complete examples of post-hooks in the [Examples](/examples/concepts/teams/pre-hooks-and-post-hooks) section.

## Post-hooks Parameters

Post-hooks will receive the `run_output` parameter, of type `RunOutput`.

This is the effective output of the Team run, after the LLM has generated the response, that you will want to validate or transform inside your pos-hook logic.

<Note>
Post-hooks are not available in streaming scenarios.
If you provide an Team with post-hook functions, they will not be executed when streaming the response.
</Note>
