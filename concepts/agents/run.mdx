...
An agent can be provided with typed input (i.e a pydantic model or a `TypedDict`) by passing it in the `Agent.run()` or `Agent.print_response()` as the `input` parameter.
...
Along with `Pydantic` models, you can also use `TypedDict` as input for agent, team or workfllow, see the [Structured Input with TypedDict](https://github.com/agno-agi/agno/blob/main/cookbook/agents/input_and_output/input_schema_on_agent_as_typed_dict.py) cookbook for more details.
</Tip>
