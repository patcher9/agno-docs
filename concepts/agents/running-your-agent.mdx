...
sidebarTitle: Running Your Agent
...
<Note>
No sensitive data is sent to the Agno servers. Telemetry is only used to improve the Agno platform and provide better support.
</Note>

Below is an example of the payload sent to the Agno servers:
```json
{
    "session_id": "123",
    "run_id": "123",
...
You can disable this by setting `AGNO_TELEMETRY=false` in your environment or by setting `telemetry=False` on the agent.
