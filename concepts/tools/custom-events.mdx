...
If you are using custom events, it will often be useful to be able to yield custom events. When streaming a response from an Agent or Team using a tool, your custom events will be yielded together with the rest of expected Agno events.

Our recommended setup is to create your custom event class extending our `CustomEvent` one:
