...
Next, let's build an Agent that can answer questions about Agno using the Agno MCP server. **This is a simple yet complete example that you can extend by connecting any MCP server**.
...
async def run_agent(message: str) -> None:
    # Define the MCP server you want to use
...
Unfortuntately, the DevEx of MCP is not great. Agno helps, but there's only so much we can do.
...
async def run_agent(message: str) -> None:
    ...
    # Manually set the session id so we can continue the same session across runs
...
Next, let's serve your Agent using the AgentOS.
