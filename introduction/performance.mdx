...
Your run-time will be mostly bottlenecked by inference, but we must do everything possible to minimize overhead, reduce memory usage, and parallelize tool calls. Our experience shows that these numbers add up quickly even at a reasonably small scale.
