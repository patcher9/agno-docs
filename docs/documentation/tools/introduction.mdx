...
def remove_item(session_state: Dict[str, Any], item: str) -> str:
    """Remove an item from the shopping list."""
    if item in session_state["shopping_list"]:
        session_state["shopping_list"].remove(item)
        return f"Removed {item}. List: {session_state['shopping_list']}"
    return f"{item} not found in the shopping list"

# Create an Agent with session state
agent = Agent(
    model=OpenAIChat(id="gpt-4o-mini"),
    # Initialize session state (default for all users)
    session_state={"shopping_list": []},
    db=SqliteDb(db_file="tmp/agents.db"),
    tools=[add_item, remove_item],
    # Use session state variables in instructions
