---
title: User Control Flows in Agents
description: Learn how to control the flow of an agent's execution in Agno. We call this "User Control Flows".
---

User control flows (Human-in-the-Loop) in Agno enable you to implement patterns where human oversight and input are required during agent execution. This is crucial for:
...
The `continue_run` method continues with the state of the agent at the time of the pause.  You can also pass the `RunOutput` of a specific run to the `continue_run` method, or the `run_id` and list of updated tools.
...
The `RunOutput` object has a list of tools and in the case of `requires_user_input`, the tools that require input will have `user_input_schema` populated.
...
- View [Cookbook](https://github.com/agno-agi/agno/tree/v2.0/cookbook/agents/human_in_the_loop)
