...
description: Learn how to cancel a team run.
---

You can cancel a run by using the `cancel_run` function on the Team.

Below is a basic example that starts an team run in a thread and cancels it from another thread, simulating how it can be done via an API. This is supported via [AgentOS](/docs/agent-os/introduction) as well.
...
For a more complete example, see [Cancel a run](https://github.com/agno-agi/agno/tree/v2.0/cookbook/teams/basic/team_cancel_a_run.py).
