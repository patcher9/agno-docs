---
title: Storage
description: Give your Agents the ability to use a database to store and retrieve their history.
sidebarTitle: Overview
---

Using **Storage**, you can enable your Agents to remember previous messages.

It works by equipping your Agents with a database that they will use to store and retrieve their [sessions](/agents/sessions) from.
...
Agents with a `db` will persist their [sessions](/agents/sessions) in the database. You can also automatically add the persisted history to the context, effectively enabling Agents to remember previous messages:
...
<Note>
Learn more about [Teams](/teams/) and [Workflows](/workflows/), Agno abstractions to build multi-agent systems.
</Note>
...
We also support using an [In-Memory](/db/in_memory) database. This is not recommended for production, but perfect for demos and testing.

You can see a detailed list of [examples](/examples/db/) for all supported databases.