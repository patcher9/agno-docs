...
The `add_content()` function is async by default. Call `add_content_sync()` if you don't want to use async.
Using `add_content()` ensures you take full advantage of the non-blocking operations, concurrent processing, and reduced latency that async vector database operations offer. This is especially valuable in production environments with high throughput requirements.
