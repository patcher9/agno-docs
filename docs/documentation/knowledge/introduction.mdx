...
The `add_content()` function is sync by default. Call `add_content_async()` if you don't want to use async.
Using `add_content_async()` ensures you take full advantage of the non-blocking operations, concurrent processing, and reduced latency that async vector database operations offer. This is especially valuable in production environments with high throughput requirements.
...
TODO: Update these into files showing how we add content from these sources
