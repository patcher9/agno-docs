...
**When to use**: Linear, deterministic processes where each step depends on the output of the previous step.
...
<Note>
  ...
  [Advanced](/docs/documentation/workflows/advanced) page.
</Note>
...
**Maximum Control**: Use a single Python function to orchestrate your entire workflow when you need complete programmatic control over execution logic.

This approach provides deterministic execution with full flexibility while maintaining workflow features like storage, streaming, and session management. Perfect for complex business logic that's easier to express in code than declarative steps.
...
For migration from 1.0 style workflows, refer to the page for [Migrating to Workflows 2.0](/docs/how-to/v2-migration)
...
**When to use**: Deterministic branching based on input analysis or business rules.
...
**When to use**: Independent, concurrent tasks that can execute simultaneously for improved efficiency.
...
**When to use**: Quality-driven processes requiring repetition until specific conditions are met.
...
### 7. Dynamic Routing Workflows

**When to use**: Complex decision trees requiring dynamic path selection based on content analysis.
...
**When to use**: Organize multiple steps into reusable, logical sequences for complex workflows with clean separation of concerns.
