...
<Note>
The `Workflow.run()` function returns a `WorkflowRunOutput` object when not streaming. 
Here is detailed documentation for [`WorkflowRunOutput](/reference/workflows/workflow_run_output).
</Note>
...
See detailed documentation in the [WorkflowRunOutputEvent](/reference/workflows/workflow-run-output-event) documentation.
