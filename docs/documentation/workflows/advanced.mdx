...
long_running_workflow = Workflow(
    ...
    steps=[
        Step(name="Research", agent=Agent(model=OpenAIChat(id="gpt-4o"), instructions="You are a helpful assistant that can research the web.")),
        Step(name="Deep Analysis", agent=Agent(model=OpenAIChat(id="gpt-4o"), instructions="You are a helpful assistant that can analyze the web.")),
        Step(name="Report Generation", agent=Agent(model=OpenAIChat(id="gpt-4o"), instructions="You are a helpful assistant that can generate a report.")),
    ]
)
...
**More Examples**:
- [Access Multiple Previous Steps Output](/examples/concept/workflows/06_workflows_advanced_concepts/access_multiple_previous_steps_output)
...
**More Examples**:
- [Pydantic Model as Input](/examples/concepts/workflows/06_workflows_advanced_concepts/pydantic_model_as_input)
...
2. **Output Processing**:
   - Each Agent processes the input using its `output_schema`
