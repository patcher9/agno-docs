...
Along with structured input, Workflows support input schema validation using Pydantic models to ensure type safety and data integrity at the workflow entry point. By defining an `input_schema` on your workflow, you can:

- **Validate Structure**: Ensure all required fields are present and correctly typed
- **Type Safety**: Automatic validation of input data against your Pydantic model
...
Set the `input_schema` parameter when creating your workflow:
...
workflow = Workflow(
    name="Content Creation Workflow",
    steps=[research_step, content_planning_step],
    input_schema=ResearchTopic,  # <-- Define expected input structure
)
```

### Valid Input Patterns

Once you define an input schema, the workflow accepts several input formats:

#### 1. Pydantic Model Instance (Recommended)
```python
# ✅ Pass a properly constructed Pydantic model
...
workflow.print_response(input=research_topic, markdown=True)
```

#### 2. Dictionary with Matching Fields
```python
# ✅ Pass a dict that matches the schema structure
workflow.print_response(
    input={
        "topic": "AI trends in 2024",
...
The workflow will raise validation errors for invalid inputs:

#### Missing Required Fields
```python
# ❌ Will fail - missing required 'target_audience' field
workflow.print_response(
    input=ResearchTopic(
        topic="AI trends in 2024",
        focus_areas=["Machine Learning", "NLP"],
        # target_audience missing - validation error
    ),
    markdown=True,
)
```
...
# ❌ Will fail - different Pydantic model provided
workflow.print_response(
...
**More Examples**:
- [Input Schema Validation](/examples/use-cases/workflows/06_workflows_advanced_concepts/workflow_with_input_schema_validation)
