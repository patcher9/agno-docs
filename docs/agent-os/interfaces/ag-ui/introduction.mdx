...
You can see more in our cookbook examples at `cookbook/agent_os/interfaces/agui/*`.

## Core Components

- `AGUI` (interface): Wraps an Agno `Agent` or `Team` into an AG-UI compatible FastAPI router.
- `AgentOS.serve`: Serves your FastAPI app with Uvicorn.
...
See cookbook examples under `cookbook/agent_os/interfaces/agui/`.
