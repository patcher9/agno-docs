...
## AgentOS
- Introducing `AgentOS`, a system for hosting agents, teams and workflows as a production-ready API. See full details in the [AgentOS](../agentos) section.
...
### Session Objects
- Session storage now stores `AgentSession`, `TeamSession` and `WorkflowSession` with new schemas. See full details in the [Session](../session) section.
...
## Knowledge
- Knowledge has been completely reworked. See full details in the [Knowledge](../knowledge) section.
...
- Import locations for `embedder`, `document`, `chunking`, `reranker` and `reader` have been moved to `agno.knowledge`. See [examples](../examples/knowledge) for more details.
...
- General:
    ...
    - A new flow allows images, audio and video files generated during tool execution to be passed back in a `FunctionExecutionResult` object and this will ensure these artifacts are made available to the model and agent as needed. See [here](../tools/function) for more details.
...
- Added support for websocket streaming of the workflow. This is appropriate for long-running workflows that need to be streamed to a client. This is only available for `arun`. See [here](../workflows/websocket) for more details.
