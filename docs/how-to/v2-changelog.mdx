...
This is a major release that introduces a completely new approach to building multi-agent systems. It is a complete rewrite of the Agno library and introduces various new concepts and updates to the existing ones.
...
## AgentOS
- Introducing `AgentOS`, a system for hosting agents, teams and workflows as a production-ready API. See full details in the [AgentOS](/docs/agent-os/introduction) section.
...
- Removed `WhatsappApi` and replace with `Slack` interface on `AgentOS`.
...
- This means that, for example, `agent.session_state` should not be accessed directly 
...
### Session Objects
- Session storage now stores `AgentSession`, `TeamSession` and `WorkflowSession` with new schemas. See full details in the [Session](/docs/documentation/agents/sessions) section.
...
## Knowledge
- Knowledge has been completely reworked. See full details in the [Knowledge](/docs/documentation/knowledge/) section.
...
- Added full `async` support for embedding models and vector DBs. This has a significant impact on performance and is a major speed improvement when adding content to the knowledge base.
...
- General:
    ...
    - A new flow allows images, audio and video files generated during tool execution to be passed back in a `FunctionExecutionResult` object and this will ensure these artifacts are made available to the model and agent as needed. See [here](/docs/documentation/tools/function) for more details.
...
- `debug_mode: bool` added to `run`, `arun`, `print_response` and `aprint_response` to enable debug mode for a specifc run.
...
- Added `get_session` to retrieve as session by ID.
...
- Workflows "v1" has been completely removed and replaced with `Workflows v2`. See full details in the [Workflows](/docs/documentation/workflows) section.
...
- Added support for websocket streaming of the workflow. This is appropriate for long-running workflows that need to be streamed to a client. This is only available for `arun`. See [here](/docs/documentation/workflows/websocket) for more details.
