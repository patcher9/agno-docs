...
<Steps>
  ...
  <Step title="Run the agent">
    ...
    ```shell
    pip install agno openai ddgs newspaper4k lxml_html_clean
    ```
...
response: RunOutput = agent.run("Simulation theory")
...
response_stream: Iterator[RunOutputEvent] = agent.run("Simulation theory", stream=True)
...
The `Agent.run()` function returns a `RunOutput` object or an stream of `RunOutput` objects if `stream=True`.
