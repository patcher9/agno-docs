...
The safest way to switch models is to change the model ID while keeping the same provider. Here's the exact pattern from the cookbook for sharing history between agents:
...
- **Message history incompatibility**: Different providers use different message formats and role structures, making conversation history transfer between providers unreliable
